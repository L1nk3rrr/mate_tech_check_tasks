...
def load_data(file_path: str) -> list[dict]:
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            data.append({
                'published_at':
                    datetime.strptime(row['Published At'],
                                      '%Y-%m-%d').date(),
                'title': row['Title'],
...
def video_with_highest_views(videos: list[dict]) -> str:
    return max(videos, key=lambda item: item['views'])['title']
...
def filter_popular_videos(videos: list[dict]) -> list[dict]:
    return [
        ...
        if video['views'] > 1_000_000 and
        video['likes'] > 500_000
    ]


def top_videos_by_category(videos: list[dict],
                           categories: list[str]
                           ) -> dict[str, list[dict]] | None:
    categories_dct = {category: [] for category in categories}
    sorted_videos = sorted(videos,
                           key=lambda item: item['views'],
                           reverse=True)
    for video in sorted_videos:
        if video['category'] in categories_dct:
            categories_dct[video['category']].append(video)
    for category, video in categories_dct.items():
        categories_dct[category] = video[:3]
...
def avg_comments_popular_videos(videos: list[dict]) -> float:
    ...
    return sum(commented_videos) / len(commented_videos)


def video_filter_generator(videos: list[dict]) -> Iterator[tuple[str, int]]:
    popular = [
        (video['title'], video['comment_count'])
        for video in videos if video['comment_count'] > 450_000
    ]
...
if __name__ == '__main__':
    ...
    # Task 1.2 Calculate the average likes-to-views
    # ratio across all videos.
    avg_ratio = average_likes_to_views_ratio(data)
    print(f"Average likes-to-views ratio: {avg_ratio: .4f}")

    # Task 1.3 Filter and return a list of videos
    # with views greater than 1,000,000 and likes
    # greater than 500,000.
    popular_videos = filter_popular_videos(data)
    print('Popular videos:', len(popular_videos))

    # Task 1.4 Group videos by category and
    # return the top 3 on each category with views number.
    top_videos = top_videos_by_category(
        data, categories=['gaming', 'tech', 'crypto'])
    for category, vids in top_videos.items():
        print(f"Category: {category}")
        for video in vids:
            print(f"  Title: {video['title']}, Views: {video['views']}")

    # Task 1.5 Find the average number of comments
    # per video for videos with views greater
    ...
    # Task 1.6 Write a generator that yields videos
    # with comment count greater than 450,000 (must return title and views)
